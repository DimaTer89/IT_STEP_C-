...
void download(FILE* out,Student *&a,int &kol) {
	char filename[20];
	cout << " Введите имя файла ввода : ";
	cin >> filename;
	if (fopen_s(&out, filename, "r") != NULL) {
	...
	char buffer[256];
	fscanf_s(out, "%d\n", &n);
	Student * grup = new Student[n];
	for (int i = 0; i < n; i++) {
		fgets(buffer, sizeof(buffer), out);
		char *word, *next;
		word = strtok_s(buffer, " \n", &next);
		strcpy_s(grup[i].lastname, word);
		word = strtok_s(NULL, " \n", &next);
		strcpy_s(grup[i].grupa, word);
		for (int j = 0; j < 5; j++) {
			word = strtok_s(NULL, " \n", &next);
			grup[i].acadPer[j] = atoi(word);
		}
	}
	fclose(out);
	delete[]a;
